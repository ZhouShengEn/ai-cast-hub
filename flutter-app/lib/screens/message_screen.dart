...
class _MessageScreenState extends ConsumerState<MessageScreen> with RouteAware {
  ...
  void didPushNext() {
    DebugService()
        .log('[MessageScreen] didPushNext → 退出浏览态', level: LogLevel.info);
  ...
  void didPopNext() {
    DebugService()
        .log('[MessageScreen] didPopNext → 重新进入浏览态', level: LogLevel.info);
    ref.read(messageProvider.notifier).setViewing(true);
  }
  ...
  Widget build(BuildContext ctx) {
    ...
    final pid =
        d.pairedDevices.isNotEmpty ? d.pairedDevices.first.deviceUuid : '';
    final pname =
        d.pairedDevices.isNotEmpty ? d.pairedDevices.first.deviceName : 'PC';
    ...
    return Scaffold(
      backgroundColor:
          theme.brightness == Brightness.light ? const Color(0xFFEDEDED) : null,
      appBar: AppBar(
        title: Text(pname),
        actions: [
          if (s.isConnected)
      ...
      body: Column(children: [
        ...
        // 状态栏（未连接且无错误）
        if (!s.isConnected &&
            (!s.isConnecting && s.error == null || pid.isEmpty))
          Container(
            width: double.infinity,
            ...
            child: Column(children: [
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text(
                    s.isConnecting
                        ? '正在连接...'
                        : (pid.isEmpty ? '请先在首页输入连接码绑定 PC 设备' : '未连接'),
                    style: TextStyle(
              ...
              if (pid.isEmpty) ...[
                const SizedBox(height: 6),
                TextButton.icon(
                  onPressed: () => Navigator.pushNamed(ctx, '/scan').then((_) =>
                      ref.read(deviceProvider.notifier).fetchDeviceList()),
                  icon: const Icon(Icons.bluetooth, size: 16),
                  label: const Text('输入连接码绑定 PC 设备',
        ...
        Expanded(
          child: s.messages.isEmpty
              ? Center(
                  child: Text(s.isConnected ? '开始聊天吧~' : '连接后可发送消息',
        ...
        if (s.isConnected)
          Container(
            ...
            padding: EdgeInsets.fromLTRB(
                8, 6, 8, MediaQuery.of(ctx).padding.bottom + 6),
            child: Row(children: [
              IconButton(
                  icon: const Icon(Icons.add_circle_outline, size: 28),
                  color: Colors.grey,
                  onPressed: () => n.sendFile()),
              Expanded(
                child: TextField(
                  controller: _ctrl,
                  maxLines: 4,
          ...
          Container(
            ...
            padding: EdgeInsets.fromLTRB(
                8, 8, 8, MediaQuery.of(ctx).padding.bottom + 8),
            child: Row(children: [
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: const Text('连接 PC 设备后可发送消息',
      ...
      child: Row(
        ...
        children: [
          if (!me) ...[
            CircleAvatar(
                radius: 18,
                backgroundColor: t.colorScheme.primary.withOpacity(0.1),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.only(
                ...
                bottomLeft:
                    me ? const Radius.circular(16) : const Radius.circular(4),
                bottomRight:
                    me ? const Radius.circular(4) : const Radius.circular(16),
              ),
            ),
  ...
  Widget _fileCard(ChatMessage m, MessageNotifier n, bool me) {
    ...
    return Semantics(
      ...
      child: GestureDetector(
        ...
        onTap: canOpen ? () => _openReceivedFile(m) : null,
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (m.isTransferring) ...[
            ...
            ClipRRect(
                ...
                child:
                    LinearProgressIndicator(value: m.progress, minHeight: 5)),
            const SizedBox(height: 4),
            Row(children: [
              Text('${(m.progress * 100).toInt()}%',
                  style: const TextStyle(fontSize: 12, color: Colors.black45)),
              if (m.speedLabel != null) ...[
                const SizedBox(width: 8),
                Text(m.speedLabel!,
                    style: const TextStyle(
                        fontSize: 12,
  ...
  Future<void> _openReceivedFile(ChatMessage message) async {
    ...
    if (!opened) {
      DebugService().warn('[MessageScreen] 文件未打开: $filePath');
    }
  }
}
