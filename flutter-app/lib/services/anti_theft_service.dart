...
///  - request_location           ：一次性上报当前坐标（Web 端「刷新定位」触发）
///  - lock_device / unlock_device：丢失模式（App 内可见提示，需本机或 Web 解除）
///
/// 合规约束：不做熄屏静默定位、不做 WakeLock 强行保活；
/// 每次远程指令都会写入本地操作日志，用户可在「设备防盗」页查看。
///
/// 单例设计：App 启动即由 [AntiTheftService]（见 app.dart）初始化并开始后台监听，
/// 即便「设备防盗」页未打开，WS 指令也能抵达并执行（配合 BackgroundConnectionService
...
class AntiTheftService {
  ...
  /// 全局单例：App 启动与 UI Provider 共用同一实例
  ...
  Future<void> _handleCommand(Map<String, dynamic> msg) async {
    ...
    switch (action) {
      ...
      case 'request_location':
        // 一次性主动上报当前坐标（Web 端「刷新定位」触发），不开启持续共享
        final ok = await _reportLocation();
        _sendAck('request_location', ok, ok ? '已上报当前位置' : '获取位置失败');
        await _log('request_location', fromUuid, ok ? '已上报当前位置' : '获取位置失败');
        break;
      case 'lock_device':
  ...
  /// 释放资源（仅在进程退出时调用；单例不随 UI Provider dispose 而销毁，
  /// 以保证后台 WS 指令监听常驻）
  void dispose() {
    _locationTimer?.cancel();
    _locationTimer = null;
